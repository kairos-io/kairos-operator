...
var _ = Describe("NodeOp Controller - Concurrency and StopOnFailure", func() {
	...
	Context("When SkipNodesAlreadyAtImage is enabled", func() {
		const targetImage = "quay.io/kairos/fedora:v4.0.3-amd64-generic-v0.0.4-k3sv1.32.4-k3s1"

		annotateNode := func(name string, annotations map[string]string) {
			node := &corev1.Node{}
		...
		It("skips Jobs for nodes whose kairos.io/image-repo annotation matches Spec.Image", func() {
			By("Annotating two of three nodes as already at the target image")
			annotateNode(nodeNames[0], map[string]string{"kairos.io/image-repo": targetImage})
			annotateNode(nodeNames[1], map[string]string{"kairos.io/image-repo": targetImage})

			By("Creating a NodeOp with SkipNodesAlreadyAtImage=true and matching Spec.Image")
			nodeOp := &kairosiov1alpha1.NodeOp{
				TypeMeta: metav1.TypeMeta{
					APIVersion: "kairos.io/v1alpha1",
					Kind:       "NodeOp",
				...
				Spec: kairosiov1alpha1.NodeOpSpec{
					...
					Image:                   targetImage,
					Concurrency:             0,
					SkipNodesAlreadyAtImage: asBool(true),
				},
			}
			Expect(k8sClient.Create(ctx, nodeOp)).To(Succeed())
			...
			By("Verifying only one Job was created (for the non-matching node)")
			jobList := &batchv1.JobList{}
			Expect(k8sClient.List(ctx, jobList,
				client.InNamespace("default"),
				client.MatchingLabels(map[string]string{"kairos.io/nodeop": resourceName}),
			)).To(Succeed())
			Expect(jobList.Items).To(HaveLen(1), "Should create only one Job for the un-annotated node")
			Expect(jobList.Items[0].Labels["kairos.io/node"]).To(Equal(nodeNames[2]))

			By("Verifying status records skipped nodes as Completed with a skip message and no JobName")
			Expect(k8sClient.Get(ctx, types.NamespacedName{
				Name:      resourceName,
				Namespace: "default",
			}, nodeOp)).To(Succeed())

			Expect(nodeOp.Status.NodeStatuses).To(HaveKey(nodeNames[0]))
			Expect(nodeOp.Status.NodeStatuses).To(HaveKey(nodeNames[1]))
			Expect(nodeOp.Status.NodeStatuses).To(HaveKey(nodeNames[2]))

			skipped0 := nodeOp.Status.NodeStatuses[nodeNames[0]]
			Expect(skipped0.Phase).To(Equal("Completed"))
			Expect(skipped0.JobName).To(BeEmpty())
			Expect(skipped0.Message).To(ContainSubstring("Skipped"))
			Expect(skipped0.Message).To(ContainSubstring(targetImage))

			skipped1 := nodeOp.Status.NodeStatuses[nodeNames[1]]
			Expect(skipped1.Phase).To(Equal("Completed"))
			Expect(skipped1.JobName).To(BeEmpty())
			Expect(skipped1.Message).To(ContainSubstring("Skipped"))

			running := nodeOp.Status.NodeStatuses[nodeNames[2]]
			Expect(running.JobName).NotTo(BeEmpty(), "Non-matching node should have a real Job")
		})

		It("does not skip Jobs when SkipNodesAlreadyAtImage is unset", func() {
			By("Annotating one node as already at the target image")
			annotateNode(nodeNames[0], map[string]string{"kairos.io/image-repo": targetImage})

			By("Creating a NodeOp WITHOUT SkipNodesAlreadyAtImage")
			nodeOp := &kairosiov1alpha1.NodeOp{
				TypeMeta: metav1.TypeMeta{
					APIVersion: "kairos.io/v1alpha1",
					Kind:       "NodeOp",
			...
			By("Verifying a Job was created for every node, including the annotated one")
			jobList := &batchv1.JobList{}
			Expect(k8sClient.List(ctx, jobList,
				client.InNamespace("default"),
				client.MatchingLabels(map[string]string{"kairos.io/nodeop": resourceName}),
			)).To(Succeed())
			Expect(jobList.Items).To(HaveLen(len(nodeNames)))
		})

		It("does not skip nodes whose kairos.io/image-repo annotation does not match Spec.Image", func() {
			By("Annotating one node with a DIFFERENT image")
			annotateNode(nodeNames[0], map[string]string{"kairos.io/image-repo": "quay.io/kairos/fedora:older-tag"})

			By("Creating a NodeOp with SkipNodesAlreadyAtImage=true but non-matching annotation")
			nodeOp := &kairosiov1alpha1.NodeOp{
				TypeMeta: metav1.TypeMeta{
					APIVersion: "kairos.io/v1alpha1",
					Kind:       "NodeOp",
			...
			By("Verifying all nodes received a Job (none skipped)")
			jobList := &batchv1.JobList{}
			Expect(k8sClient.List(ctx, jobList,
				client.InNamespace("default"),
				client.MatchingLabels(map[string]string{"kairos.io/nodeop": resourceName}),
		...
		It("rolls overall Phase up to Completed when every target node is skipped", func() {
			By("Annotating all nodes as already at the target image")
			for _, n := range nodeNames {
				annotateNode(n, map[string]string{"kairos.io/image-repo": targetImage})
			}

			By("Creating a NodeOp with SkipNodesAlreadyAtImage=true")
			nodeOp := &kairosiov1alpha1.NodeOp{
				TypeMeta: metav1.TypeMeta{
					APIVersion: "kairos.io/v1alpha1",
					Kind:       "NodeOp",
			...
			By("Verifying no Jobs were created at all")
			jobList := &batchv1.JobList{}
			Expect(k8sClient.List(ctx, jobList,
				client.InNamespace("default"),
				client.MatchingLabels(map[string]string{"kairos.io/nodeop": resourceName}),
			...
			By("Verifying NodeOp phase is Completed")
			Expect(k8sClient.Get(ctx, types.NamespacedName{
				Name:      resourceName,
				Namespace: "default",
			}, nodeOp)).To(Succeed())
